...
{
	/** Note constants */
	public static final int R10 = 10, R20 = 20, R50 = 50, R100 = 100,
			R200 = 200;
	...
	public int[] dispense(int amount)
	{
		...
		if ((amount < 0) || (amount % R10 != 0))
			throw new IllegalArgumentException("Illegal amount entered + R"
					+ amount + ".00");
	...
	public int getSingleNoteCount(final int note)
	{
		...
		throw new IllegalArgumentException(
				"Invalid note amount! (Tip: Use Dispenser.R -constants)");
	}
	...
	private int dispense(int amount, int startIndex, int[] record)
	{
		...
		if (amount >= curNoteValue)
		{
			...
			int rem = dispense(amount - curNoteValue * multiples,
					startIndex - 1, record);
			if (rem == 0)
			{
	...
	{ // one main() to test them all!
		Dispenser disp = new Dispenser();
		while (true)
		{
			String ans = JOptionPane
					.showInputDialog(null, "Balance: R" + disp.getBalance()
							+ ".00\nEnter amount:", "");
			if (ans == null)
				break;
			...
			for (int i = 0; i < notesValues.length; ++i)
				System.out.println("\tR" + notesValues[i] + ": "
						+ disp.notesCount[i]);
		}
	}
}
